...
function AccessContext(context) {
  ...
  this.model = ('string' === typeof this.model) ? loopback.getModel(this.model) : this.model;
  if (context.resolverContext) {
    const { resolverContext } = context;
    this.model = resolverContext.model;
    this.data = resolverContext.data;
    this.query = resolverContext.query;
    this.orgId = resolverContext.orgId;
    this.accessToken = resolverContext.accessToken;
    this.modelId = resolverContext.modelId;
  } else if (context.remotingContext && context.remotingContext.req) {
    const { remotingContext } = context;
    this.data = remotingContext.req.body;
    this.query = remotingContext.req.query;
    this.accessToken = remotingContext.req.accessToken && remotingContext.req.accessToken.toObject();
    this.orgId = remotingContext.req.orgId;
  }
  this.modelName = this.model && this.model.modelName;

  this.modelId = this.modelId || context.id || context.modelId;
  ...
  this.accessToken = this.accessToken || context.accessToken || loopback.AccessToken.ANONYMOUS;
...
AccessContext.prototype.addPrincipal = function(principalType, principalId, principalName) {
  ...
  for (var i = 0; i < this.principals.length; i++) {
    ...
    if (!(p instanceof Principal)) {
      p = new Principal(p.type, p.id, p.name);
    }
